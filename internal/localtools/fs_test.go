...
func TestLocalReadAllowsFilesInsideRoot(t *testing.T) {
	...
	result := Handle(context.Background(), RootedFS{Root: root}, "local_fs_read", json.RawMessage(`{"path":"notes.txt"}`))
	if !result.Success {
...
func TestLocalReadCapsOutputLength(t *testing.T) {
	...
	result := Handle(context.Background(), RootedFS{Root: root}, "local_fs_read", json.RawMessage(`{"path":"large.txt","max_bytes":10}`))
	if !result.Success {
		t.Fatalf("read failed: %s", result.Output)
	}
...
func TestLocalWritePatchAndDeleteRequirePathInsideRoot(t *testing.T) {
	...
	write := Handle(context.Background(), RootedFS{Root: root}, "local_fs_write", mustJSON(t, map[string]any{
		"path":    "dir/file.txt",
		"content": "hello world",
	}))
	if !write.Success {
	...
	patch := Handle(context.Background(), RootedFS{Root: root}, "local_fs_patch", mustJSON(t, map[string]any{
		"path":     "dir/file.txt",
		"old_text": "world",
		"new_text": "conan",
	}))
	...
	blocked := Handle(context.Background(), RootedFS{Root: root}, "local_fs_write", mustJSON(t, map[string]any{
		"path":    outside,
		"content": "blocked",
	}))
	...
	del := Handle(context.Background(), RootedFS{Root: root}, "local_fs_delete", json.RawMessage(`{"path":"dir/file.txt"}`))
	if !del.Success {
...
func TestLocalListAndStat(t *testing.T) {
	...
	list := Handle(context.Background(), RootedFS{Root: root}, "local_fs_list", json.RawMessage(`{"path":"."}`))
	if !list.Success || !strings.Contains(list.Output, "a.txt") {
		t.Fatalf("list = %+v", list)
	}

	stat := Handle(context.Background(), RootedFS{Root: root}, "local_fs_stat", json.RawMessage(`{"path":"a.txt"}`))
...
func TestLocalFilesystemRejectsBinaryAndImageFiles(t *testing.T) {
	...
	readImage := Handle(context.Background(), RootedFS{Root: root}, "local_fs_read", json.RawMessage(`{"path":"image.png"}`))
	if readImage.Success || !strings.Contains(readImage.Output, "binary/image") {
		t.Fatalf("read image result = %+v", readImage)
	}

	readBinary := Handle(context.Background(), RootedFS{Root: root}, "local_fs_read", json.RawMessage(`{"path":"binary.txt"}`))
	if readBinary.Success || !strings.Contains(readBinary.Output, "binary content") {
		t.Fatalf("read binary result = %+v", readBinary)
	}

	writeImage := Handle(context.Background(), RootedFS{Root: root}, "local_fs_write", mustJSON(t, map[string]any{
		"path":    "new.png",
		"content": "text",
	}))
	...
	writeBinary := Handle(context.Background(), RootedFS{Root: root}, "local_fs_write", mustJSON(t, map[string]any{
		"path":    "new.txt",
		"content": "a\x00b",
	}))
	...
	patchBinary := Handle(context.Background(), RootedFS{Root: root}, "local_fs_patch", mustJSON(t, map[string]any{
		"path":     "binary.txt",
		"old_text": "a",
		"new_text": "c",
	}))
	...
	deleteImage := Handle(context.Background(), RootedFS{Root: root}, "local_fs_delete", json.RawMessage(`{"path":"image.png"}`))
